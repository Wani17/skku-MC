...
void randomTest(int numItems, int dim, int numQueries, int K, int numThreads, int workload) {
	...
	// 인자 조정 진행함
	HNSWGraph myHNSWGraph(12, 30, 30, 13, 4, numItems, randomItems[0], numThreads);

	// insert 진행
	#pragma omp parallel for num_threads(numThreads) schedule(static, 100)
	for (int i = 1; i < numItems; i++) {
		if (i % 10000 == 0) cout << "." << std::flush;
		myHNSWGraph.Insert(randomItems[i], i);
	}

	// pruning을 진행해서 query에 소요되는 시간 단축
	#pragma omp parallel for num_threads(numThreads)
	for (int i = 0; i < numItems; i++) {
		myHNSWGraph.Pruning_noLock(i);
	}
	cout << endl;
